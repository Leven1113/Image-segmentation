...
for i=1:ceil(NoSnakeIterations/5),
   if i<=floor(NoSnakeIterations/5) 
      [x,y] = snakedeform(x,y,alpha,beta,gamma,kappa,px,py,5);%/zjg/x,y是目标周围演化的曲线，是由点组成。px,py是外力区域，当传到snakedeform()后变为fx,fy形参/zjg/
...
hold on
if SnakeON==1 %如果SnakeON==1,画一个封闭的曲线即snake
   XS=[XSnake; XSnake(1)];%XSnake(1)是XSnake向量的第一个元素，在原向量的末尾再加一个头向量，这样做就是一个头尾相接的坐标，目标是画出一个封闭曲线。
   YS=[YSnake; YSnake(1)];
   HDline=line(XS, YS); 
end
...
function CloseFig()
close(get(gcbo,'UserData'));%gcbo返回调用这个函数的figure的handle，应该是f1
return
