...
'''
Draw polygon regions of interest (ROIs) in matplotlib images, similar to Matlab's roipoly function.
Created by Joerg Doepfert 2014 based on code posted by Daniel Kornhauser.
'''
# from image_manipulation import get_screen_size
...
class Roipoly:
    ...
    def __init__(self, fig=[], ax=[], roicolor='b'):
        if fig == []:
            fig = plt.gcf()
            # [w, h] = get_screen_size(70)  # how many percent of the screen size is required
...
image_grayscale = rgb2gray(plt.imread('temp.jpg'))
plt.imshow(image_grayscale, cmap='gray')
my_roi = Roipoly(roicolor='r')
mask = my_roi.get_mask(image_grayscale)
plt.imsave(arr=mask, fname='mask.jpg', cmap=cm.gray)